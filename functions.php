...
if(!function_exists('register_sidebar') && get_option('tarski_sidebar_type') == 'widgets') {
	update_option('tarski_sidebar_type', '');
}

// set default sidebar type
if(!get_option('tarski_sidebar_type')) {
	// default to widgets if available, otherwise use the Tarski sidebar
	if(function_exists('register_sidebar')) {
		update_option('tarski_sidebar_type', 'widgets');
	} else {
		update_option('tarski_sidebar_type', 'tarski');
	}
}
...
function tarski_bodyclass() {
	if(get_option('tarski_centered_theme')) { // Centred or not
		echo 'center';
	} else {
		echo 'left';
	}
	if(get_option('tarski_swap_sides')) { // Swapped or not
		echo ' janus';
	}
	if(get_option('tarski_style')) { // Alternate style
...
function tarskiupdate() {
	...
	if ( !empty($_POST) ) {
		...
		update_option('tarski_sidebar_type', $_POST['sidebartype']);
	}
}
...
if (!get_option('tarski_installed')) {
	...
	add_option('blurb', __('This is the about text', 'tarski'), __('Allows you to write a small blurb about you and your blog, which will be put on the front page', 'tarski'), $autoload);
}
