...
<?php } elseif(!get_option('tarski_sidebar_onlyhome') || !(is_single() || is_page())) { ?>

<div class="secondary">
<?php @include(TEMPLATEPATH . '/constants.php'); echo $sidebarTopInclude; ?>

<?php if(get_option('tarski_sidebar_type') == 'widgets') { ?>
	<div class="widgets">
		<?php dynamic_sidebar(__('Main Sidebar', 'tarski')); ?>
	</div>
<?php } elseif(get_option('tarski_sidebar_type') == 'tarski') { ?>
	<?php if($sidebar_custom = get_option('tarski_sidebar_custom')) {
		echo '<div class="content">' .  wpautop(wptexturize(stripslashes($sidebar_custom))) . '</div>';
	} ?>
	
	<?php if(get_option('tarski_sidebar_pages')) { // pages block ?>
	...
	<?php if(get_option('tarski_sidebar_comments') && function_exists('blc_latest_comments')) { // comments block ?>
	...
	<?php } // end links block ?>
<?php } elseif(get_option('tarski_sidebar_type') == 'custom') { ?>
