...
if(get_option('tarski_update_notification') == 'true') { ?>
...
<form name="dofollow" action="" method="post">
	...
	<input type="hidden" name="page_options" value="'dofollow_timeout'" />
		...
		<div class="section">
			<fieldset class="primary">
				...
				<?php if(get_option('tarski_update_notification') == 'true') { ?>
			...
			<fieldset class="secondary">
				<h3><?php _e('About Text','tarski'); ?></h3>
				<textarea name="about_text" rows="5" cols="30" id="about_text"><?php echo stripslashes(get_option('blurb')); ?></textarea>
		...
		<div class="section">
			<fieldset class="primary radiolist">
				...
				<label for="option-ts"><input type="radio" id="option-ts" name="sidebartype" value="tarski" onchange="$('tarski-sidebar-section').show(); $('widgets-sidebar-section').hide(); $('custom-sidebar-section').hide();"<?php if(get_option('tarski_sidebar_type') == 'tarski') { echo " checked=\"checked\""; } ?> /> <?php _e('Tarski sidebar options','tarski'); ?></label>
				<?php if(function_exists('register_sidebar')) { ?>
					<label for="option-ws"><input type="radio" id="option-ws" name="sidebartype" value="widgets" onchange="$('widgets-sidebar-section').show(); $('tarski-sidebar-section').hide(); $('custom-sidebar-section').hide();"<?php if(get_option('tarski_sidebar_type') == 'widgets') { echo " checked=\"checked\""; } ?> /> <?php _e('WordPress Widgets','tarski'); ?></label>
				<?php } else { ?>
				...
				<?php } ?>
				<label for="option-fs"><input type="radio" id="option-fs" name="sidebartype" value="custom" onchange="$('custom-sidebar-section').show(); $('widgets-sidebar-section').hide(); $('tarski-sidebar-section').hide();"<?php if(get_option('tarski_sidebar_type') == 'custom') { echo " checked=\"checked\""; } ?> /> <?php _e('Alternate sidebar file','tarski'); ?></label>
				
				<label class="spaced-out" for="opt-sidebar-onlyhome">
					<input type="hidden" name="sidebar[onlyhome]" value="0" />
					<input type="checkbox" name="sidebar[onlyhome]" value="1"  id="opt-sidebar-onlyhome" <?php if(get_option('tarski_sidebar_onlyhome')) { echo 'checked="checked" '; } ?>/>
					<?php _e('Only display the sidebar on index pages','tarski'); ?>
				</label>
				<p><?php _e('The home, archive and search pages are index pages; this option is good for people with long sidebars.')?></p>
			</fieldset>

			<fieldset class="secondary">
			<h3><?php _e('&hellip;and configure it.','tarski'); ?></h3>
			
				<div id="tarski-sidebar-section" class="insert"<?php if(get_option('tarski_sidebar_type') != 'tarski') { echo ' style="display: none;"'; } ?>>
					<label for="opt-sidebar-pages">
						<input type="hidden" name="sidebar[pages]" value="0" />
						<input type="checkbox" name="sidebar[pages]" value="1" id="opt-sidebar-pages" <?php if(get_option('tarski_sidebar_pages')) { echo 'checked="checked" '; } ?>/>
	...
	<?php if(function_exists('blc_latest_comments')) { ?>
					<label for="opt-sidebar-comments">
						...
						<input type="checkbox" name="sidebar[comments]" value="1" id="opt-sidebar-comments" <?php if(get_option('tarski_sidebar_comments')) { echo 'checked="checked" '; } ?>/>
						<?php _e('Latest comments','tarski'); ?>
					</label>
	<?php } ?>
					<label for="opt-sidebar-links">
						<input type="hidden" name="sidebar[links]" value="0" />
						<input type="checkbox" name="sidebar[links]" value="1" id="opt-sidebar-links" <?php if(get_option('tarski_sidebar_links')) { echo 'checked="checked" '; } ?>/>
					...
					<textarea name="sidebar[custom]" rows="5" cols="30" id="sidebar_custom"><?php echo stripslashes(htmlspecialchars(get_option('tarski_sidebar_custom'))); ?></textarea>
				</div>
			
			
				<div id="widgets-sidebar-section" class="insert"<?php if(get_option('tarski_sidebar_type') != 'widgets') { echo ' style="display: none;"'; } ?>>
				...
				<div id="custom-sidebar-section" class="insert"<?php if(get_option('tarski_sidebar_type') != 'custom') { echo ' style="display: none;"'; } ?>>
		...
		<div class="section">
			<fieldset class="primary">			
				...
				<h3><?php _e('Header Image','tarski'); ?></h3>
					...
					if ($header_dir) {
						...
						if ($header_dir || $header_images) {
							...
							foreach($header_images as $header_image) {
								...
								if(str_replace("-thumb", "", $header_image) == get_option('tarski_header')) {
					...
					<input type="hidden" name="header_image" id="header_image" value="<?php echo stripslashes(get_option('tarski_header')); ?>" />
			...
			<fieldset class="secondary">
				...
				<?php
				global $wpdb;
				$id = get_option('tarski_asidescategory');
				if ($id != 0) {
				...
				?>
					<select name="asides_category" id="asides_category">
						<option value="<?php echo get_option('tarski_asidescategory'); ?>"><?php echo $asides_title; ?></option>
						<option value="-----">----</option>
				...
				$nav_pages = explode(',', get_option('tarski_nav_pages'));
				...
				<label for="opt-misc-title">
					...
					<input type="checkbox" id="opt-misc-title" name="display_title" value="1" <?php if(get_option('tarski_display_title') != 'lolno') { echo 'checked="checked" '; } ?>/>
					<?php _e('Display site title','tarski'); ?>
				</label>

				<label for="opt-misc-tagline">
					<input type="hidden" name="display_tagline" value="0" />
					<input type="checkbox" id="opt-misc-tagline" name="display_tagline" value="1" <?php if(get_option('tarski_display_tagline')) { echo 'checked="checked" '; } ?>/>
					<?php _e('Display site tagline','tarski'); ?>
				</label>

				<label for="opt-misc-cats">					
					<input type="hidden" name="hide_categories" value="0" />
					<input type="checkbox" id="opt-misc-cats" name="hide_categories" value="1" <?php if(get_option('tarski_hide_categories')) { echo 'checked="checked" '; } ?>/>
					<?php _e('Hide post categories','tarski'); ?>
				</label>
				
				<label for="opt-misc-pagination">
					<input type="hidden" name="use_pages" value="0" />
					<input type="checkbox" id="opt-misc-pagination" name="use_pages" value="1" <?php if(get_option('tarski_use_pages')) { echo 'checked="checked" '; } ?>/>
				...
				<label for="opt-misc-centre">						
					...
					<input type="checkbox" id="opt-misc-centre" name="centered_theme" value="1" <?php if(get_option('tarski_centered_theme')) { echo 'checked="checked" '; } ?>/>
				...
				<label for="opt-misc-tagsearch">
					...
					<input type="checkbox" id="opt-misc-tagsearch" name="ajax_tags" value="1" <?php if(get_option('tarski_ajax_tags')) { echo 'checked="checked" '; } ?>/>
				...
				<label for="opt-misc-janus">	
					...
					<input type="checkbox" id="opt-misc-janus" name="swap_sides" value="1" <?php if(get_option('tarski_swap_sides')) { echo 'checked="checked" '; } ?>/>
